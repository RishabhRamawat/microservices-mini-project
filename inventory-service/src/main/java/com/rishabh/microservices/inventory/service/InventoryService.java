...
public class InventoryService {
    ...
    public InventoryResponse decrementStock(String sku, InventoryAdjustmentRequest request) {
        if (request.quantity() == null || request.quantity() <= 0) {
            throw new IllegalArgumentException("Decrement quantity must be a positive integer");
        }

        Inventory inventory = inventoryRepository.findBySku(sku)
                .orElseThrow(() -> new InventoryNotFoundException("Inventory not found for SKU: " + sku));

        // Reject before touching stock so quantity never goes negative
